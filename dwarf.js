!function (root) {
        ...
        return {
            ...
            make: function (src) {
                stack.forEach(function (o) {
                    ...
                    var module = o.m = _normalize(src, o.m); // 拼接

                    var loader = Cache.get(module); // 拿到module
                    if (!loader) { // 如果没有module 就是没有被加载 就new一个放在Cache
    ...
    Loader.prototype = {
        ...
        load: function (url) {
            ...
            function _onload() {
                _onend();// 删除之前的load 避免重复触发
                return Def.make(self.path); // 对应真正的url传回去
            }
            function _onerror() {
                _onend(); 
